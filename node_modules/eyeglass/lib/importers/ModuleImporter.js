    ...
    try {
        for (var possibleFiles_1 = __values(possibleFiles), possibleFiles_1_1 = possibleFiles_1.next(); !possibleFiles_1_1.done; possibleFiles_1_1 = possibleFiles_1.next()) {
            ...
            try {
                ...
                var data = buildCache.get("fs.readFileSync(" + nextFile + ")");
...
function readFirstFileCached(buildCache, uri, files, callback) {
    var readCacheKey = files.join(";");
    var countKey = "readAbstractFile(" + readCacheKey + ")-count";
    var invocationCount = buildCache.get(countKey);
    ...
    if (invocationCount === 1) {
        ...
        var fileKey_1 = "readAbstractFile(" + readCacheKey + ")-file";
        var file = buildCache.get(fileKey_1);
        var contents = void 0;
        var contentsKey_1;
        if (file) {
            contentsKey_1 = "fs.readFileSync(" + file + ")";
            contents = buildCache.get(contentsKey_1);
        }
        if (file && contents) {
            callback(null, { file: file, contents: contents });
        }
        else {
            readFirstFile(buildCache, uri, files, function (err, data) {
                if (data && !err) {
                    buildCache.set(fileKey_1, data.file);
                    contentsKey_1 = "fs.readFileSync(" + data.file + ")";
                    buildCache.set(contentsKey_1, data.contents);
                }
                callback(err, data);
            });
        }
...
function readAbstractFile(originalUri, uri, location, includePaths, moduleName, buildCache, callback) {
    ...
    var files = new (Array.bind.apply(Array, __spread([void 0], nameExpander.files)))();
...
var ModuleImporter = function (eyeglass, sass, options, fallbackImporter) {
    ...
    var buildCache = options.eyeglass.buildCache;
    return ImportUtilities_1.default.createImporter("module", function (uri, prev, done) {
        ...
        var isRealFile = perf_1.existsSync(prev);
